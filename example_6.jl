...
L = 6.; #length of horizontal electrode
n_haste = 2; # Number of rods
distX = 6.0; # Distance between vertical rods
prof = 0.5; #Depth
CONDX, CONDY, CONDZ, Node, A, S = MC_Vertical_Rods(r,L,n_haste, distX,prof,20*r); #getting data
ρ = 100.0; #soil resistivity
ϵr = 9.0; #soil permitivity
nInt = 20;#points in gauss legandre
#Excitation data   -------------------------------------------------------------
...
ZP = (maximum(v_t[1:end-10])/maximum(current));
plot(10^6*t,10^-3*v_t, label = "HEM", linecolor = :black,lw = 2, xlabel = "Time [μs]",ylabel = "GPR [kV]");
plot!(10^6*t,10^-3*current*ZP, label = "ZP", linecolor = :red,lw = 2, xlabel = "Time [μs]",ylabel = "GPR [kV]");
savefig("Frequency_Response"*Nome*".png");
println("The impulsive impedance is equal to: $ZP")
cd(dirname(@__FILE__))
println("\n");
