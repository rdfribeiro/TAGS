...
function HEM(ro::Float64,epsr::Float64,rf::Float64,CONDUTORESx,CONDUTORESy,CONDUTORESz,A,S,freq,nInt::Int64,SoilParameter::String)
    ...
    for line = 1:(nseg)
        ...
        for colum = line:(nseg)
            ...
            dele = sqrt((XS1-XS0)^2 + (YS1-YS0)^2 + (ZS1-ZS0)^2); #DIFERENCIAL DO dl sqrt(x'(t)^2+y'(t)^2 ... CONFERIR EM COMO RESOLVER INTEGRAL DE LINHA PARA COMPREENDER
            Dot = ((XS1-XS0)*(XR1-XR0)+(YS1-YS0)*(YR1-YR0)+(ZS1-ZS0)*(ZR1-ZR0))/(dele^2); #Projeção (associado com produtor vetorial entre "dl's")
            ...
            Intl = 0.25*(aux1r + Tal_rl*aux1i)*dele^2*Dot; #AINDA TEM QUE COLOCAR CONSTANTE DE REFLEXÃO
            Intt = 0.25*(aux1r + Tal_rt*aux1i)*dele^2;

            Zl[line,colum] = (1im*w*mi0/(4*pi))*Intl; #ARMAZENANDO IMPEDANCIA LONGITUDINAL
            Zt[line,colum] = (1/(4*pi*(sigma+1im*w*eps)*dele^2))*Intt; #ARMAZENANDO IMPEDANCIA TRANSVERSAL
