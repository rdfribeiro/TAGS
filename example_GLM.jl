...
L = 20.; #length of each counterpoise (must be greater than 10.0)
d = 20.; #distance between counterpoises
prof = 0.5; #Grounding grid depth
CONDX, CONDY, CONDZ, Node, A, S = MC_Torre_2(r,L,prof,10*r); #getting data
ρ = 100.0; #soil resistivity
ϵr = 9.0; #soil permitivity
freq = logspace(log10(100),log10(2e6),20); #frequency points
nfreq = length(freq); #number of points
nInt = 10;#points in gauss legandre
Z = zeros(nfreq) + im*zeros(nfreq); #inicializating the impedance
Zl = zeros(nfreq) + im*zeros(nfreq); #inicializating the impedance
...
begin time_GLM = @elapsed Threads.@threads for linha = 1:nfreq
        Yn = GLM(prof,d,L,r,ρ,ϵr,freq[linha],"");
        if Threads.threadid() == 1
            println("$(100.0*linha/nfreq*ncores) % concluído ... Aguarde")
        end
        Zl[linha] = ((inv(Yn))[1,1]); # obtaining the Z of the first point, corner
        global Zl
    end
end
#saving data
...
plot!(freq,abs.(Zl), label = "Transmission Line Model", xscale = :log10, linecolor = :red,lw = 2,xlabel = "Frequency [Hz]",ylabel = "|z(ω)|");
savefig("Frequency_Response"*Nome*".png");
#saving data in a .mat structure
file = matopen("Grounding_Data.mat", "w"); write(file, "Zl", Zl);
write(file, "Z", Z); write(file, "freq", freq); close(file);
cd(dirname(@__FILE__))
println("\n");
println("Total time (HEM): $(time_original)\n")
println("Total time (GLM): $(time_GLM)\n")
